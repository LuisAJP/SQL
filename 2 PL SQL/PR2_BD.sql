...
IF CONT_PEDIDOS = 0 THEN
  DBMS_OUTPUT.PUT_LINE('EL CLIENTE CON DNI: ' || p_DNI || 'NO HA EFECTUADO NINGÚN PEDIDO');
ELSE
...
DECLARE
  BEGIN
  PEDIDOS_CLIENTE('12345679M'); /*NO EXISTE*/
  END;
END;
---------------------------------APARTADO_02-------------------------------------------------------------------
...
-- Procedimiento del apartado 3 que utiliza la funcion anterior (Calcular_Total_Pedido) y actualiza el importe de los pedidos si no están bien
